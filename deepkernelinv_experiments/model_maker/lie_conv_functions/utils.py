...
def conv2d(in_channels,out_channels,kernel_size=3,coords=False,dilation=1,**kwargs):
    ...
    return layers.Conv2d(in_channels,out_channels,kernel_size,padding=same,dilation=dilation,**kwargs)
...
class GaussianNoise(tf.Module):
    ...
    def forward(self, x):
        if self.training:
            return x + tf.random.normal(x.shape)*self.std
        else:
            return x
...
class Pass(layers.Layer):
    def __init__(self, module, dim=1):
        ...
        self.module = module
        self.dim=dim
        assert dim == 1, f"dim unequal 1 not implemented yet"


    def build(self, input_shape):
        assert len(input_shape) == 3, f"only imlemented for 3-tuples currently"
        self.module.build(input_shape[self.dim])
        return
    # def call 
    ...
    def call(self,x):
        ...
        locs, charges, mask = x
        new_charges = self.module[charges]
        x = locs, new_charges, mask
        return x
...
def square_distance(src, dst):
    ...
    if src.shape[1]==1 or dst.shape[1]==1:
        return tf.sum((src - dst) ** 2, -1)
    #print(src.shape,dst.shape)
    B, N, _ = src.shape
    _, M, _ = dst.shape
    dist = -2 * tf.matmul(src, dst.permute(0, 2, 1))
    dist += tf.sum(src ** 2, -1).view(B, N, 1)
    dist += tf.sum(dst ** 2, -1).view(B, 1, M)
    return dist
...
def index_points(points, idx):
    ...
    repeat_shape[0] = 1
    batch_indices = tf.keras.arange(B, dtype=tf.int64).to(device).view(view_shape).repeat(repeat_shape)
...
def farthest_point_sample(xyz, npoint, distance=square_distance):
    ...
    device = xyz.device
    B, N, C = xyz.shape
    centroids = tf.zeros(B, npoint, dtype=tf.int64).to(device)
    distances = tf.ones(B, N).to(device) * 1e8
    farthest = tf.random.uniform(0, N, (B,), dtype=tf.int64).to(device)
    batch_indices = tf.keras.arange(B, dtype=tf.int64).to(device)
    for i in range(npoint):
        ...
        #farthest = torch.max(distances, -1)[1]  #Pola: torch.max returns max, argmax 
...
def knn_point(nbhd, xyz, new_xyz, mask, distance=square_distance):
    ...
    _, group_idx = tf.math.topk(sqrdists,nbhd, dim = -1, largest=False, sorted=False)
    if tf.reduce_any.any(group_idx>10000): # This means there was an error, nans in input?
...
@export
class FarthestSubsample(tf.Module):
